...
export class DocumentSharesController {
  ...
  @ApiOperation({
    summary: "Partager un lien d'invitation par email",
  })
  ...
  async shareFile(
    ...
    await this.documentSharesService.shareEmailLink(
      userReq.id,
      documentId,
      body.invitedEmail,
    );
  ...
  @ApiOperation({
    summary:
      "Accepter une invitation de partage via un token et rediriger vers l'application",
  })
  @ApiOkResponse({
  ...
  acceptInvite(
    ...
    try {
      ...
      this.logger.error(
        "Une erreur s'est produite lors de l'acception de l'invitation: ",
        err,
      );
  ...
  async acceptInviteInApp(
    ...
    await this.documentSharesService.acceptInvite(
      userReq.id,
      userReq.email,
      body.token,
    );
