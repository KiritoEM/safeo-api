...
export class DocumentService {
    ...
    async uploadFile(
        ...
        const spaceStatus = await this.userRepository.checkStorageStatus(
            user.id,
            file.size,
        );
        ...
        if (spaceStatus == UserStorageStatusEnum.INSUFFICIENT) {
            throw new PayloadTooLargeException(
                'Espace insuffisant pour mettre le fichier dans le cloud',
            );
        ...
        const DekEncryptionKeyPayload =
            this.encryptionKeyService.generateAESDek(kekKey);

        if (!DekEncryptionKeyPayload) {
            throw new InternalServerErrorException(
                'Impossible de créér la clé de chiffrement',
            );
        }
        ...
        const createdDocument = await this.documentRepository.create({
            ...
            fileMimeType: file.mimetype,
            encryptedMetadata: formatEncryptedData(
                encryptedMetadataPayload.IV,
                encryptedMetadataPayload.encrypted,
                encryptedMetadataPayload.tag,
            ),
            encryptedKey: formatEncryptedData(
                DekEncryptionKeyPayload.IV,
                DekEncryptionKeyPayload.encrypted as string,
                DekEncryptionKeyPayload.tag,
            ),
            accessLevel,
        ...
        const { encryptedKey, encryptedMetadata, fileName, ...filteredDocument } =
            createdDocument[0];
    ...
    async getAllDocuments(
        ...
        const documentsWithPublicUrl = allDocuments.map(
            async (doc) =>
    ...
    async getSharedDocuments(
        ...
        // audit log
        await this.logRepository.log({
            action: AUDIT_ACTIONS.GET_USER_SHARED_DOCUMENT,
            target: AUDIT_TARGET.DOCUMENT,
            userId,
            ipAddress,
        });
    ...
    async decryptDocumentMetadata<T extends Document>(
        KekKeyPlain: string,
        doc: T,
        ...
        // Decrypt metadata
        const decomposedEncryptedMetadataPayload = decomposeEncryptedData(
        ...
        const { bucketPath } = JSON.parse(decryptedMetadata) as Record<
            string,
            string
        >;
        ...
        const cachedPublicUrl = (await this.cache.get(
            'document:url:' + doc.id,
        )) as { publicUrl: string };
    ...
    async deleteDocument(userId: string, documentId: string, ipAddress?: string) {
        //get user by id
        const user = await this.userRepository.findUserById(userId);
        ...
        const deletedDocument = await this.documentRepository.softDelete(
            documentId,
            user.id,
        );
        ...
        await this.userRepository.update(user.id, {
            storageUsed: user.storageUsed! - deletedDocument[0].fileSize,
        });
